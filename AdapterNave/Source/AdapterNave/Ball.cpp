...
ABall::ABall()
{
	...
	static ConstructorHelpers::FObjectFinder<UStaticMesh> BallMesh(TEXT("StaticMesh'/Game/Geometry/Meshes/Sphere.Sphere'"));
	static ConstructorHelpers::FObjectFinder<UMaterialInstance> MaterialBall(TEXT("MaterialInstanceConstant'/Game/TwinStick/Meshes/MterialBall.MterialBall'"));
	...
	CollisionBall->SetCapsuleRadius(0.50f);
	CollisionBall->SetCapsuleHalfHeight(0.50f);
...
void ABall::Launch()
{
	//if (!BallLaunched) 
	//{
		SM_Ball->AddImpulse(FVector(-7200.0f, 0.0f, 1.0f), FName(), true);
		//BallLaunched = true;
